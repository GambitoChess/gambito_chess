...
class Scacchiera(): #scrivo la mia matriciona, i nomi dei pezzi sono i nomi delle immagini da caricare
	def __init__(self):
		...
		self.arroccoBiancoLungo = True
		self.varco_poss = 8 #8: non permessa, valore minore di 8 indica su quale colonna è permessa.
		self.click_in_out = []
    	    
	def controlloColoreMossa(self, mossa): #controlla che il pezzo mosso sia del giocatore che ha il tratto
	...
	def muovere(self, mossa): #funzione che modifica la matriciona
		if self.controlloColoreMossa(mossa) and self.mossaValida(mossa):
			...
			self.muoveilBianco = not self.muoveilBianco #se muove il bianco mo tocca al nero e viceversa
	...
	def mossaValida(self, mossa): #controllo che il pezzo selezionato possa muovere come indicato
		...
		risposta = False #sarà il discrimante, alla fine dove c'è via libera metti questo risposta = True e qui risposta = False
		...
		for event in py.event.get(): #crea una lista di eventi possibili (e.g. click del mouse)
			...
			if event.type == py.MOUSEBUTTONDOWN: #se l'utente clicca il mouse
				...
				if len(gs.click_in_out) == 2: #se l'utente clicca la seconda casa
				    move = Mossa(gs.click_in_out[0], gs.click_in_out[1], gs.board) #eseguo la mossa
				    print(move.NotazioneBrutta()) #stampo la notazione a schermo (per controllare tutto vada bene
				    gs.muovere(move)
				    selezione_casa = () #setto tutto a zero
				    gs.click_in_out = []
